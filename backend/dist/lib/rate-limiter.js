...
exports.getSenderHourlyCount = getSenderHourlyCount;
exports.getDelayUntilNextHour = getDelayUntilNextHour;
...
function getHourKey(senderId, date = new Date()) {
...
function getNextHourStart(date = new Date()) {
 ...
 */
async function checkAndIncrementRateLimit(senderId, hourlyLimit) {
    ...
    if (count === 1) {
        await redis_1.redis.expire(key, HOUR_SECONDS + 60);
    }
 ...
 */
async function getSenderHourlyCount(senderId) {
    const key = getHourKey(senderId);
 ...
 */
function getDelayUntilNextHour(from = new Date()) {
